<script lang="ts">
	...
	import { preloadStyled, type CornerDotStyle, type CornerSquareStyle, type DotStyle } from '$lib/styled';
	...
	<div class="mt-4 grid gap-5">
		{#if design.halftoneActive}
			<p class="notice notice-info">
				Artistic (halftone) mode is on, and it replaces these settings. Dot shapes, gradients, the logo, and the frame are
				ignored while a picture is blended in.
			</p>
		{/if}
		...
		<div class="field">
			...
			{#if design.frameEnabled}
				<div class="grid gap-3">
					<input class="input" type="text" aria-label="Frame text" bind:value={design.frameText} maxlength="40" list="cta-list" />
					...
					<p class="hint">Specific wording ("Scan for the menu") gets more scans than "Scan me".</p>
				</div>
			{/if}
		</div>
	</div>
